            ...
            @foreach($steps as $step)
                @if ($step->isPrevious())
                    <li class="relative pb-10">
                        <div class="absolute left-4 top-4 -ml-px mt-0.5 h-full w-0.5 bg-red" aria-hidden="true"></div>
                        <!-- Complete Step -->
                        <a href="#" class="group relative flex items-start">
                        <span class="flex h-9 items-center">
                            <span class="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-red group-hover:bg-red">
                ...
                @elseif ($step->isCurrent())
                    <li class="relative pb-10">
                        ...
                        <span class="flex h-9 items-center" aria-hidden="true">
                            <span class="relative z-10 flex h-8 w-8 items-center justify-center rounded-full border-2 border-red bg-white">
                            <span class="h-2.5 w-2.5 rounded-full bg-red"></span>
                            </span>
                        </span>
                        <span class="ml-4 flex min-w-0 flex-col">
                            <span class="text-sm font-medium text-red">{{ $step->label }}</span>
                            <span class="text-sm text-gray-500">{{ $step->description }}</span>
                        </span>
                        </a>
                    </li>
